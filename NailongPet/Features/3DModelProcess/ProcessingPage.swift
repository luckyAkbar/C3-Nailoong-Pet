...
struct ProcessPage: View {
    ...
    private var progressPercentage: Float {
        ...
        case .mlSharp:
            if case .completed = sharpViewModel.state { return 100 }
    ...
    private var targetProgress: Double {
        ...
        case .mlSharp:
            ...
            case .idle: return 0
            case .processing(let progress, _): return min(Double(progress * 100), 99)
            case .completed: return 100
            case .failed: return displayedProgress
            }
        }
    }
    ...
    private var phaseText: String {
        ...
        case .mlSharp:
            switch sharpViewModel.state {
            case .idle: return "Preparing Sharp model..."
            case .processing(_, let phase): return phase
            case .completed: return "Done. Preparing next step..."
            case .failed: return ""
            }
        }
    }
    ...
    var body: some View {
        ...
        .onChange(of: targetProgress) { _ in startProgressAnimation() }
        .onChange(of: sharpViewModel.state) { _ in scheduleAutoAdvanceIfNeeded() }
    }
    ...
    private func startProgressAnimation() {
        ...
        progressTask = Task {
            ...
            if generatorType == .mlSharp && displayedProgress == 0 && target > 0 {
                await MainActor.run {
                    withAnimation(.easeOut(duration: 0.2)) { displayedProgress = 1.5 }
                }
            }
            ...
            while !Task.isCancelled {
                ...
                if remaining <= 0.1 {
                    await MainActor.run { withAnimation(.easeOut(duration: 0.18)) { displayedProgress = target } }
                    break
                }
                let step = max(0.35, remaining * 0.08)
                let delay: UInt64
                switch target {
                case ..<25: delay = 70_000_000
                case ..<75: delay = 110_000_000
                default: delay = 160_000_000
                }
                await MainActor.run { withAnimation(.linear(duration: Double(delay) / 1_000_000_000)) { displayedProgress = min(current + step, target) } }
    ...
    private func scheduleAutoAdvanceIfNeeded() {
        ...
        Task { @MainActor in
            withAnimation(.easeOut(duration: 0.2)) { displayedProgress = 100 }
            try? await Task.sleep(nanoseconds: 650_000_000)
            router.navigate(to: .processPetDetail(GeneratorType.mlSharp))
        }
    }
        ...
        VStack(spacing: 32) {
            ...
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 10)
                    .stroke(.white.opacity(0.4), style: StrokeStyle(lineWidth: 1.5, dash: [6, 5]))
...
struct ProcessPage_Previews: PreviewProvider {
