...
struct SharpImageSelectionView: View {
            ...
            VStack(spacing: 0) {
                ...
                Button(action: {
                    if let uiImage = sharpViewModel.selectedImage {
                        Task { await sharpViewModel.process(uiImage: uiImage) }
                        router.navigate(to: .processPage(GeneratorType.mlSharp))
                    }
                }) {
                    Text("Create 3D")
                        .font(.subheadBold)
    ...
    private var backgroundLayer: some View { Color.surfaceCanvas }
}

struct SharpImageSelectionView_Previews: PreviewProvider {
